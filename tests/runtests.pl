...
#######################################################################
# initialize global variables needed to set up servers
#
serverinitglobals($verbose, $PIDDIR, $LOGDIR, $srcdir, $FTPDCMD, $perl, $debugprotocol,
    $err_unexpected, $proxy_address, $use_external_proxy, $VCURL,
    $torture, $stunnel, @protocols);
init_serverpidfile_hash();

if(!$listonly) {
    # these can only be displayed after serverinitglobals() has been called
