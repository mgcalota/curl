...
# Copies of test-global variables in runtests.pl
# These are initialized in serverinitglobals()
my $verbose;
my $PIDDIR;
my $LOGDIR;
my $srcdir;
...
# This sets up local copies of widely-used variables that are awkward
# to access by dereferencing with main:: all the time.
# TODO: find a better way. How about a serverconf module that just has these
# global vars?
sub serverinitglobals {
  ($verbose, $PIDDIR, $LOGDIR, $srcdir, $FTPDCMD, $perl, $debugprotocol,
   $err_unexpected, $proxy_address, $use_external_proxy, $VCURL,
   $torture, $stunnel, @protocols)=@_;
